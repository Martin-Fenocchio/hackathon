...
export class OrchestratorController {
  ...
  @Post('orchestrate')
  async orchestrateTransferText(
    @Body() orchestrateTransferTextDto: OrchestrateTransferTextDto,
  ...
  async orchestrateConfirmTransfer(@Body() body: { telephone: string }, @Res() res: Response): Promise<void> {
    /* const { voucherImage } = await this.orchestratorService.orchestrateConfirmTransfer(body.telephone);

    res.set({
      'Content-Type': 'image/png',
      'Content-Disposition': `attachment; filename="transfer-voucher-.png"`,
    });
